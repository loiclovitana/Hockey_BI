...
def import_manager(repository_session: RepositorySession,
                   user_email: str) -> models.Manager:
...
def import_team(repository_session: RepositorySession,
                manager: models.Manager, team_code: str,
                players_ids: list[int], at_datetime: datetime = None):
    ...
    current_season_team = repository_session.get_team(manager, current_season, team_code)
    current_players = [current_team_player.player_id for current_team_player in current_season_team]
    for current_team_player in current_season_team:
        if current_team_player.from_datetime is not None and current_team_player.from_datetime >= at_datetime:
    ...
    new_team_player = [
        models.Team(team=team_code, manager_id=manager.id
                    , player_id=player_id
